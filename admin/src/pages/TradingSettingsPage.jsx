...
const DEFAULT_TRADING = {
  ...
  minWithdrawal: 10,
  defaultLeverage: 1,
  leverageOptions: '1,10,20,50,100,200,500',
  commissionPerLot: 5,
  swapLong: -0.5,
  swapShort: 0.25,
  spreadMarkup: 0.5,
...
export default function TradingSettingsPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SECTION A — existing trading category settings
      ══════════════════════════════════════════════════════════════════ */}

      {/* Deposit & Withdrawal Settings */}
      <Card>
        <SectionTitle icon="💰" title="Deposit & Withdrawal Settings" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            type="number"
            value={tradingForm.minWithdrawal}
            onChange={(e) => setT('minWithdrawal', e.target.value)}
            fullWidth
            hint="Clients cannot request withdrawals below this amount"
          />
        </div>
      </Card>
      ...
      {/* Leverage Settings */}
      <Card>
        <SectionTitle icon="⚡" title="Leverage Settings" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            label="Default Leverage"
            type="number"
            value={tradingForm.defaultLeverage}
            onChange={(e) => setT('defaultLeverage', e.target.value)}
            fullWidth
            hint="Default leverage for new accounts"
          />
          <div>
            ...
            <textarea
              value={tradingForm.leverageOptions}
              onChange={(e) => setT('leverageOptions', e.target.value)}
              ...
              rows="2"
              placeholder="1,10,20,50,100,200,500"
            />
      ...
      {/* Commission Settings */}
      <Card>
        <SectionTitle icon="🏦" title="Commission Settings" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            label="Commission per Lot (USD)"
            type="number"
            step="0.01"
            value={tradingForm.commissionPerLot}
            onChange={(e) => setT('commissionPerLot', e.target.value)}
            fullWidth
          />
        </div>
      </Card>
      ...
      {/* Swap Settings */}
      <Card>
        <SectionTitle icon="🔄" title="Swap Settings" subtitle="Overnight interest charged/paid on open positions" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            label="Long Position Swap (%)"
            type="number"
            step="0.01"
            value={tradingForm.swapLong}
            onChange={(e) => setT('swapLong', e.target.value)}
            fullWidth
          />
          <Input
            label="Short Position Swap (%)"
            type="number"
            step="0.01"
            value={tradingForm.swapShort}
            onChange={(e) => setT('swapShort', e.target.value)}
            fullWidth
          />
        </div>
      </Card>
      ...
      {/* Spread Settings */}
      <Card>
        <SectionTitle icon="📊" title="Spread Settings" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            label="Spread Markup (Pips)"
            type="number"
            step="0.01"
            value={tradingForm.spreadMarkup}
            onChange={(e) => setT('spreadMarkup', e.target.value)}
            fullWidth
          />
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Input
            label="Max Positions per Account"
            type="number"
            value={tradingForm.maxPositions}
            onChange={(e) => setT('maxPositions', e.target.value)}
            fullWidth
          />
          <Input
            label="Margin Call Level (%)"
            type="number"
            value={tradingForm.marginCallLevel}
            onChange={(e) => setT('marginCallLevel', e.target.value)}
            fullWidth
          />
          <Input
            label="Stop Out Level (%)"
            type="number"
            value={tradingForm.stopOutLevel}
            onChange={(e) => setT('stopOutLevel', e.target.value)}
            fullWidth
          />
        </div>
      </Card>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SECTION B — MT5 Login Series (category: mt5)
      ══════════════════════════════════════════════════════════════════ */}
      <Card>
        <SectionTitle
          icon="🔢"
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SECTION C — Market Access Permissions (category: permissions)
      ══════════════════════════════════════════════════════════════════ */}
      <Card>
        <SectionTitle
          icon="🌍"
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SECTION D — Account Creation Markets
      ══════════════════════════════════════════════════════════════════ */}
      <Card>
        <SectionTitle
          icon="🏦"
          title="Account Creation Markets"
          subtitle="Control which market segments users are allowed to open new trading accounts in"
        />

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          {[
            {
              key:   'account_create_forex',
              label: 'Forex + Crypto',
              hint:  'Users can create accounts for Forex pairs, crypto and global indices',
              icon:  '💱',
              color: 'blue',
            },
            {
              key:   'account_create_comex',
              label: 'Comex',
              hint:  'Users can create accounts for Comex commodities (metals, energy)',
              icon:  '🛢️',
              color: 'amber',
            },
            {
              key:   'account_create_mcx_nse',
              label: 'MCX + NSE',
              hint:  'Users can create accounts for Indian commodity (MCX) and stock (NSE) markets',
              icon:  '🇮🇳',
              color: 'green',
            },
          ].map(({ key, label, hint, icon, color }) => {
            const enabled = !!permForm[key]
            return (
              <div
                key={key}
              ...
              >
                ...
                <p className={`text-sm font-semibold mb-1 ${
                  enabled ? `text-${color}-700 dark:text-${color}-300` : 'text-dark-700 dark:text-dark-300'
                }`}>
                  {label}
                </p>
                <p className="text-xs text-dark-500 dark:text-dark-400">{hint}</p>
                <div className={`mt-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                  ...
                  <span className={`w-1.5 h-1.5 rounded-full ${enabled ? `bg-${color}-500` : 'bg-dark-400'}`} />
        ...
        <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-300">
          ⚠️ Disabling a market will hide that option from the "Create Account" page for all users. Existing accounts in that market are not affected.
        </div>
      </Card>

      {/* ══════════════════════════════════════════════════════════════════
          SECTION E — Trading Rule Permissions
      ══════════════════════════════════════════════════════════════════ */}
      <Card>
        <SectionTitle
          icon="📋"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {[
            {
              key: 'allow_scalping',
              label: 'Allow Scalping',
              hint: 'Permit trades held for very short durations (under 60 seconds)',
            },
            {
              key: 'allow_news_trading',
              label: 'Allow News Trading',
              hint: 'Permit opening/closing trades during high-impact news events',
            },
            {
              key: 'allow_hedging',
              label: 'Allow Hedging',
              hint: 'Permit simultaneous buy & sell on the same symbol',
            },
            {
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SECTION E — Copy Trading Permissions
      ══════════════════════════════════════════════════════════════════ */}
      <Card>
        <SectionTitle
          icon="📡"
